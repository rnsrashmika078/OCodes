...
import { FilePath, Tree, UpdateChat } from "@/lib/types/type";
...
const Sidebar = ({ toggleSidebar, isToggle }: Props) => {
  const authUser = useEditor((store) => store.authUser);
  const activeChat = useEditor((store) => store.activeChat);
  const setActiveChat = useEditor((store) => store.setActiveChat);
  const setNotification = useEditor((store) => store.setNotification);
  const updateChats = useEditor((store) => store.updateChats);
  const deleteChat = useEditor((store) => store.deleteChat);
  const setUserMessages = useEditor((store) => store.setUserMessages);
  const setAuthUser = useEditor((store) => store.setAuthUser);
  const chats = useEditor((store) => store.chats);
