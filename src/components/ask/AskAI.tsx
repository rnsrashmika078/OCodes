...
const AskAI = ({ toggleSidebar }: ASKAI) => {
  ...
  const setUpdateProjectFile = useChatClone(
    (store) => store.setUpdateProjectFile
  );
...
-> If history is empty, use this -> ${activeFile?.content}. This contains the currently open file code.
